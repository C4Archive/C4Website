...
Class PageData {
  ...
  static function is_parent($base_url, $permalink) {
    $base_path = preg_replace('/^[^\/]+/', '', $base_url);
    if($permalink == 'index') {
      return FALSE;
    } else {
      return strpos($_SERVER['REQUEST_URI'], $base_path.'/'.$permalink) === 0;
    }
  }

  static function is_grandparent($base_url, $permalink) {
    $parent_path = preg_replace('/^[^\/]+/', '', $base_url);
    $base_path = preg_replace('/^[^\/]+/', '', $parent_path);
    if($permalink == 'index') {
      return FALSE;
    } else {
      return strpos($_SERVER['REQUEST_URI'], $base_path.'/'.$permalink) === 0;
  ...
  static function create_vars($page) {
	...
	# @is_parent
	$page->is_parent = self::is_parent($page->data['@base_url'], $page->data['@permalink']);
	# @is_grandparent
	$page->is_grandparent = self::is_grandparent($page->data['@base_url'], $page->data['@permalink']);
    # @is_last
